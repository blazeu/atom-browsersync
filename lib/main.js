...
export default {
  ...
  activate () {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      ...
      'atom-browsersync:toggle-proxy': () => this.toggle(true),
  ...
  toggle (proxy) {
    ...
    if (proxy) {
      let textEditorDialog = new TextEditorDialog('Enter url to proxy.')
      textEditorDialog.attach((url) => {
        this.initBs(pathObj, url)
      })
    } else {
      this.initBs(pathObj)
    }
